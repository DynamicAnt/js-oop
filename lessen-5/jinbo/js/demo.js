...
Template.prototype.updateField = function ($this) {
    ...
    for(var i=0; i< $this.find('.js-save-info').length; i++){
        arr.push($this.find('.js-save-info').eq(i).val());
    }
    this.data = arr;
}
...
$.extend(Tool.prototype, {
    ...
    getTemplate: function () {
        ...
        var jsLiBox = $('.js-li-box').detach().find('li')
        var temLi1 = jsLiBox.eq(0);
        var temLi2 = jsLiBox.eq(1);
        var temLi3 = jsLiBox.eq(2);
        var jsTabBox = $('.js-tab-box').detach().children()
        var temTab1 = jsTabBox.eq(0);
        var temTab2 = jsTabBox.eq(1)
        var temTab3 = jsTabBox.eq(2)
        var temLiBox = [temLi1,temLi2,temLi3];
        var temTabBox = [temTab1,temTab2,temTab3];
        console.log(temLiBox,temTabBox)


        this.options.$ModShowIndex.map(function (t) {
            console.log(temLiBox[t],'ss')
            $('.js-tab').append(temLiBox[t]);
            $('.js-node').append(temTabBox[t]);
            _self.templateArr.push(arr[t]);
            console.log($('.js-tab')[0])
        })
    ...
    initEvent: function () {
        var _self = this;
        //点击产品标题输入框弹出模板弹窗；
        this.options.$prodName.on('click',function(){
        ...
        $('.js-tab').on('click','li', function () {
            ...
            _self.templateArr[num].updateScoreView();
        })
    },
    btnEvent: function () {
        var _self = this;
        ...
        $('.js-btn-submit').click(function(){
            var tem = _self.templateArr[_self.tabIndex];
            if(tem.isValid()){
                _self.options.$prodName.val(tem.data.join(''));
                $('.js-alpha,.js-prod-name-pop').hide();
            }
        })
        //点击取消按钮
        $('.js-btn-reset').click(function(){
            for(var i = 0;i<_self.templateArr.length;i++){
                _self.templateArr[i].reset();
            }
            _self.chooseTab(0);
