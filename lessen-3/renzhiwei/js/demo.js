$(function () {
    ...
    var template1 = new Template1(publicObj.$tab.eq(0));
    var template2 = new Template2(publicObj.$tab.eq(1));
    var template3 = new Template3(publicObj.$tab.eq(2));
    ...
    $('.js-submit-btn').on('click', function () {
        ...
        $tab.find('input[type="text"]').each(function () {
            if($(this).closest('td').hasClass('required')) {
                flag = template[publicObj.index].validInputMsg($(this)) &&
                     flag;
            }
        });
        $tab.find('select').each(function () {
            flag = template[publicObj.index].validSelectMsg($(this)) && flag;
        });
        $tab.find('[type="radio"]').each(function() {
            flag = template[publicObj.index].validRadioMsg($(this)) && flag;
        });
        $tab.find('.js-year').each(function() {
            flag = template[publicObj.index].validYearMsg($(this)) && flag;
        });
        if (flag) {
            var propsName = $tab.find('.js-names').text();
            publicObj.$prodName.val(propsName);
            template[publicObj.index].closePop();
        }
    });
    ...
    $('.js-cancel-btn').on('click', function () {
        ...
        publicObj.$prodMod.find('li').removeClass('on').eq(0).addClass('on');
        publicObj.$prodMod.find('.js-tab').removeClass('on').eq(0).addClass('on');
    ...
    //点击产品标题输入框弹出模板弹窗；
    publicObj.$prodName.on('click', function () {
